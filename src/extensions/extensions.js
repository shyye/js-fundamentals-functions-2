...
function calculateQuantities(ingredientList, cakeLayers) {
  ...
  if (ingredientList.includes('sugar')) {
    object.sugar = 100 * cakeLayers
  }

  if (ingredientList.includes('eggs')) {
    object.eggs = 2 * cakeLayers
  }
...
function improveRecipe(cakeObject, numberOfPortions) {
  const ingredients = Object.keys(cakeObject)
  ingredients.forEach((ingredient) => {
    cakeObject[ingredient] = cakeObject[ingredient] * numberOfPortions
  })
  return cakeObject
}
